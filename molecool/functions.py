...
def calculate_distance(rA, rB):
    ...
    distance = (rA - rB)
    distance_normalized = np.linalg.norm(distance)
...
def write_xyz(file_location, symbols, coordinates):
    ...
    if num_atoms != len(coordinates):
        raise ValueError(f"""write_xyz : the number of simbols ({num_atoms}) and 
            number of coordinates ({len(coordinates)}) must be the same to write xyz
            file!
            """)
